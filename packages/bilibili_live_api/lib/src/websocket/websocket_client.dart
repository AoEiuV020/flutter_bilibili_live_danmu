...
class BilibiliWebSocketClient {
  ...
  void _sendHeartbeat() {
    ...
    _sendPacket(packet);
    debugPrint('[WebSocket] 发送心跳 seq: ${packet.sequenceId}');
  }
  ...
  void _handleHeartbeatReply(ProtoPacket packet) {
    ...
    if (packet.body.length >= 4) {
      ...
      debugPrint('[WebSocket] 心跳回复, 在线人数: $online');
    }
  }
  ...
  void _handleMessageReply(ProtoPacket packet) {
    try {
      ...
      if (packet.version == ProtocolVersion.json) {
        ...
        // TODO: 处理压缩消息
        debugPrint('[WebSocket] 收到压缩消息，暂不支持');
      }
    } catch (e) {
      debugPrint('[WebSocket] 处理消息失败: $e');
    }
  ...
  void _handleError(dynamic error) {
    ...
    debugPrint('[WebSocket] 错误: $errorMsg');
